...
import Modal from './Specific Design/Modal'
    ...
    return (
        ...
        >
            <Link
                ...
                state={{
                    thumbnail, DesignData, index, feet

                }}
                className='flex flex-col items-center justify-center gap-4'
            >
                <div className="absolute z-50 flex items-center justify-center w-12 h-12 bg-white cursor-pointer top-4 right-4 rounded-xl" onClick={handleToggle}>
                    {isLiked
                        ?
                ...
                <span className="flex items-center justify-start w-full px-3 text-lg font-medium text-gray-800 lg:text-xl">{title}</span>
            </Link>
            ...
            <div className={`flex ${windowWidth<350? 'flex-col gap-4' : 'flex-row'} justify-around w-full my-2`}>
                <button className={`bg-white rounded-3xl border-[#EB595F] ${windowWidth<350? 'text-sm' : 'text-xs'} px-2 sm:px-1   py-2  border-[1px] text-[#EB595F] font-semibold  sm:text-xs md:text-sm md:px-3 lg:px-1 xl:px-3  2xl:px-6 tracking-wider  whitespace-nowrap`} onClick={handleClick}>Book Free Consultations</button>
                <button className={`bg-[#EB595F] text-white ${windowWidth<350? 'py-2 text-sm' : 'text-xs'} px-8 sm:px-3 md:px-4 xl:px-10  sm:py-2 font-medium lg:text-sm whitespace-nowrap sm:text-sm 2xl:px-12 rounded-3xl`} onClick={handleClick}>Get Quote</button>
            </div>
