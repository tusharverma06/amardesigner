...
import DesignCard2 from './DesignCard2.tsx';
...
const DesignCategory: FC<DesignCategoryProps> = ({ windowWidth }) => {
  const { designCategory } = useParams();

  return (
    <div>
      <div className='flex flex-col items-center justify-center w-full gap-12 mx-auto max-w-[2000px]'>
        {/* Heading section */}
        <section className='flex flex-col items-center justify-start w-5/6 gap-4 my-12'>
          <div className="relative flex items-center justify-between w-full gap-2">
            ...
            <h1 className='text-2xl font-bold text-start sm:text-4xl lg:text-6xl'>{designCategory}</h1>
            {/* Navigation */}
          ...
          <p className='w-full text-sm font-medium leading-6 text-left sm:text-lg md:text-xl'>
        ...
        <section className='flex flex-col items-center justify-center w-5/6 gap-12 mb-36 md:gap-10 lg:gap-5 xl:gap-12 sm:flex-row sm:flex-wrap sm:gap-y-8 lg:gap-y-16 '>
          {DesignIdeas1.map((idea, index) => (
            <DesignCard2
              title={idea.title}
              key={index}
              thumbnail={idea.thumbnail}
              feet={parseInt(idea.feet)}
